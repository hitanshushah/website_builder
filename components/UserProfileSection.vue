        ...
        <div class="flex-1 w-full ml-4">
          <div class="flex items-start justify-between">
            <div class="flex-1">
              ...
              <div v-if="userProfile.city || userProfile.province || userProfile.country" class="flex items-center gap-2 text-gray-600 dark:text-gray-400">
                <i class="pi pi-map-marker text-sm"></i>
                <span class="text-sm">{{ [userProfile.city, userProfile.province, userProfile.country].filter(Boolean).join(', ') }}</span>
              </div>
            </div>
            <!-- Links on the right -->
            <div v-if="userProfile.links && userProfile.links.length > 0" class="flex flex-wrap gap-2 ml-4 !h-fit flex-row">
              <Button
