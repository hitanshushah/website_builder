<template>
  <div class="fixed inset-0 z-50 flex items-start justify-center bg-black/50 overflow-y-auto">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg md:w-1/2 w-full p-6 my-8 mx-4">
      <div class="mb-3">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Add Details</h3>
        <p class="text-sm text-gray-600 dark:text-gray-400">Enter your information</p>
      </div>
      
      <UForm :state="state" :validate="validate" @submit.prevent="submitForm" @keydown.enter.prevent="" class="space-y-3">
        <!-- Phone Number, Secondary Email, and Empty Column -->
...
const submitForm = async () => {
  try {
    ...
    const response = await $fetch<{ success: boolean; profile: any }>('/api/contact-details', {
      method: 'POST',
      body: contactData
    })
    ...
    if (err.statusCode === 400) {
      ...
      error.value = err.data?.message || err.message || 'Failed to save contact details'
    }
  } finally {
    loading.value = false
  }
